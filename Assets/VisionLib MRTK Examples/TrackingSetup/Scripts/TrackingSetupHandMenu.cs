...
namespace Visometry.VisionLib.SDK.MRTK.Examples
{
    ...
    /// </summary>
    /// @ingroup Examples
    [HelpURL(DocumentationLink.APIReferenceURI.Examples + "tracking_setup_hand_menu.html")]
    ...
    public class TrackingSetupHandMenu : MonoBehaviour
    {
        ...
        [SerializeField]
        private string saveConfigurationURI =
            "capture-dir:VisionLib/savedTrackingParameterConfiguration.vl";
        ...
        {
            var trackingConfiguration = FindObjectOfType<TrackingConfiguration>();
            if (trackingConfiguration == null)
            {
                throw new Exception(
                    "No TrackingConfiguration found in scene. Cannot save current configuration.");
            }
            trackingConfiguration.SaveCurrentTrackerConfiguration(this.saveConfigurationURI);
        }
